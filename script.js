...
async function fetchWeatherData(city) {
    try {
        ...
        // Используем город "как есть" для API, без каких-либо преобразований
        ...
        // Используем название города из API или сохраняем оригинальное название
return fetchWeatherByCoords(lat, lon, geoData[0].local_names?.ru || geoData[0].name || city);
    } catch (error) {
        console.warn('Ошибка запроса данных о погоде:', error.message);
        
        // Если запрос был отменен, сообщаем об ошибке
...
async function fetchWeatherByCoords(lat, lon, cityName) {
    try {
       ...
       // Используем название города из ответа API или то, что было передано
// Не переопределяем его никакими хардкодными значениями
...
function handleSearch() {
   try {
       ...
       // Проверка на валидные символы (разрешаем буквы, цифры, пробелы и некоторые знаки)
       if (!/^[a-zA-Zа-яА-ЯёЁ0-9\s\-,.'()\[\]]+$/.test(city)) {
       ...
       setTimeout(() => {
           // Загружаем данные для введенного города напрямую
